...
class ScalingBreakdown:
    ...
    def build_breakdown_csv(self, single_network_directory: str, machine_range: list[str], output: str):
        ...
        # print([x.split("/")[-1][0] for x in output_files])

        output_files = list(filter(lambda x: x.split("/")[-1][0] == 'a', output_files))
        output_files = list(filter(lambda x: "LT" not in x.split("/")[-1].split('_') and str(int(builder.get_machines_truncated(x)) - 1) in machine_range, output_files))

        trials = [{**self.deconstruct_ouput_file(self.load_output_file(output_file)), **{'WorldSize': str(int(builder.get_machines_truncated(output_file))-1)}} for output_file in output_files]
        # print(trials)
    ...
    def build_receiver_breakdown_csv(self, single_network_directory: str, machine_range: list[str], output: str):
        ...
        output_files = builder.get_all_files(single_network_directory, lambda x: x.endswith(".o"))
        output_files = list(filter(lambda x: x.split('/')[-1][0] == 'a' and "LT" not in x.split("/")[-1].split('_') and str(int(builder.get_machines_truncated(x)) - 1) in machine_range, output_files))
...
def main():
    ...
    scaling_breakdown.build_breakdown_csv(sys.argv[1] + "/orkut_big", [str(int(math.pow(2, x))) for x in range(6, 10 + 1)], sys.argv[2] + "/orkut_big_timings.csv")

    scaling_breakdown.build_receiver_breakdown_csv(sys.argv[1] + "/orkut_big", [str(int(math.pow(2, x))) for x in range(6, 10 + 1)], sys.argv[2] + "/orkut_big_receiver_timings.csv")
